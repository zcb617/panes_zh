# Panes 电脑操作 SDK 阶段 2：统一服务和 Codex 工具接入

> 本记录按实施顺序编号；业务方案中的“阶段 1：统一服务和 Codex”对应本文件。
...
- 设置页“添加 exe”产品路径的替换；该项属于总体方案阶段 3。
- Claude 和 OpenCode 的工具适配；该项属于总体方案阶段 2。
...
本阶段复用已有 `computer-control-approval-requested` 事件和确认窗口，新增字段保持向后兼容；设置页和授权文案的产品化调整留在总体方案阶段 3。
...
进入总体方案阶段 2：Claude/OpenCode 适配；之后进入总体方案阶段 3，迁移 Panes 设置页开关、移除旧 exe 授权心智并完成 Windows 实机业务验收。
...
- TODO-006 的 Windows 测试宿主仍处于“阻断”，应在总体方案阶段 2 开始前再尝试一次；若环境仍然阻断，转入阶段 5 CI 回归并保留失败证据。
