...
删除产品概念上的“允许操作的应用 / 添加 exe”。旧配置字段可以在迁移阶段保留读取兼容，但不再作为授权依据，也不再写回新的应用白名单。
...
旧版本中的 `allowed_applications`、`broker`、外部 driver 路径和引擎 MCP 配置只做兼容识别，不再参与新的授权判断。迁移时：

- 保留旧字段的读取代码以避免旧配置解析失败。
...
- 移除“添加 exe”和外部 driver 检测的产品路径；旧代码按项目规则保留兼容注释或迁移分支，不直接删除。
