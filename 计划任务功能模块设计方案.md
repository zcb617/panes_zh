...
- 点击托盘图标：显示并聚焦主窗口。
- 托盘菜单“打开 Panes”：显示主窗口。
- 托盘菜单“退出 Panes”：如果“启用中”列存在任务，先提示退出后任务将无法按计划执行，并要求用户二次确认；用户取消时保持进程与调度器运行，用户确认后再释放终端、保持唤醒组件和调度器资源并退出进程。“未启用”和“需确认”列不触发该提示。
...
8. “启用中”列有任务时从托盘退出，验证取消后进程继续运行、确认后进程退出；只有“未启用”或“需确认”任务时不弹提示。
9. 退出并重新启动 Panes，验证任务恢复且不重复执行。
10. 归档或删除目标会话，验证任务进入“需确认”。
